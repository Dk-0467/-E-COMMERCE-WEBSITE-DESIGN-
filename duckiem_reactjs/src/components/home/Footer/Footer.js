...
const Footer = () => {
  ...
  const [menuFooter, setMenuFooter] = useState([]); // State để lưu menu footer

  // Lấy dữ liệu menu từ API
  useEffect(() => {
    const fetchMenuFooter = async () => {
      try {
        const result = await MenuServices.footer();
        console.log("Dữ liệu menu footer từ API:", result);
        if (result.status === true) {
          setMenuFooter(result.menu);
        }
      } catch (error) {
  ...
  return (
    <div className="w-full bg-[#F5F5F3] py-20">
      <div className="max-w-container mx-auto grid grid-cols-1 md:grid-cols-2 xl:grid-cols-6 px-4 gap-10">
        ...
        {/* Menu Footer Section */}
        <div>
          <FooterListTitle title="Menu Footer" />
          <ul className="flex flex-col gap-2">
            {menuFooter.length > 0 ? (
              menuFooter.map((menuItem) => (
                <li
                  key={menuItem.id} // Sử dụng id từ menuItem
                ...
                >
                  <a href={menuItem.link}>{menuItem.name}</a> {/* Dùng menuItem.name thay vì menuItem.title */}
                </li>
              ))
            ) : (
              <li className="text-lightText">Loading menu...</li>
            )}
          </ul>
        </div>
